...
ESP_LOGI(TAG, "M5Core2_Init Init().");

#if CONFIG_SOFTWARE_UI_SUPPORT
    M5Core2_PMU_Init(3300, 0, 0, 2700);
    M5Core2_Display_Init();
#else
    M5Core2_PMU_Init(0, 0, 0, 0);
#endif
...
void M5Core2_PMU_SetPowerIn(uint8_t mode) {
    if (mode) {
...
void M5Core2_Buzzer_Init() {
    M5Core2_Buzzer_InitWithPin(GPIO_NUM_2);
}
...
void M5Core2_Buzzer_Play() {
    M5Core2_Buzzer_Play_Duty(LEDC_DUTY);
}

void M5Core2_Buzzer_Stop() {
